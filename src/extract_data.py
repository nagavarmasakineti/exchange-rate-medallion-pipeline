...
def save_to_bronze(raw_payload):
    ...
    try:
        ...
        conn.commit()
        logger.info("Raw payload successfully loaded")
...
def validate_and_clean_data(base, target, rate):
    ...
    if(len(base)!= 3 or len(target)!=3):
        logger.warning(f"Validation Failed: Invalid Currency Code format: {base} ---> {target}")
        return False
    #Rule 2: Rate should be greater than zero
    if(rate <= 0):
        logger.warning(f"Validation Failed: Impossible exchange rate detected: {rate}")
        return False
    return True
...
def main():
    try:
        ...
        logger.info("Data Saved Successfully to BRONZE")
        ...
        if validate_and_clean_data(base, "EUR", rate):
            ...
            logger.error("Pipline Stopped: Data Failed Validation Quality Checks.")
            print("Pipline Stopped: Data Failed Validation Quality Checks.")

    except Exception as e:
        logger.error(f"Pipeling failed: {str(e)}", exc_info=True)
